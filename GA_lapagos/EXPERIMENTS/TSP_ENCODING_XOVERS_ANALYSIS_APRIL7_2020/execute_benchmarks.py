...
with open(sys.argv[1],"r") as f:
    for line in f:
        ...
        execution = sys.argv[3] + " -c " + sys.argv[2] + line.rstrip() + " -t " + sys.argv[4] + filename + ".test_out " + "-m0" 
        print("Execution: %s" % execution)

        p = subprocess.call([sys.argv[3], "-c",  sys.argv[2]+line.rstrip(),  "-t", sys.argv[4]+filename+".test_out", "-m0"], stdout=log_file, stderr=log_file)
